...
import { Image, Database, BarChart3, Eye, SlidersHorizontal, Layers } from 'lucide-react';
...
export function MultimodalCalculator({ mode = 'inference' }: MultimodalCalculatorProps) {
  ...
  const hasModalityConfig =
    config.modalityType?.includes('image') ||
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8">
      ...
      >
        ...
        {/* 子 Tab：基础配置 / 模态配置 */}
        <div className="flex gap-0 mb-5 rounded-xl overflow-hidden border border-white/20">
          <button
          ...
          >
            ...
            模态配置
            {hasModalityConfig && (
              <span className="ml-1 w-1.5 h-1.5 rounded-full bg-cyan-500 inline-block" />
            )}
          </button>
        </div>
            ...
            >
              {!hasModalityConfig && (
                <div className="text-center py-10 text-gray-500 text-sm">
                  当前模态类型（{config.modalityType}）无需额外配置
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium flex justify-between">
                      ...
                      <span className="font-mono text-cyan-600">{config.imageResolution}×{config.imageResolution}</span>
                    </label>
                    <Select 
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium flex justify-between">
                      ...
                      <span className="font-mono text-cyan-600">{config.patchSize}×{config.patchSize}</span>
                    </label>
                    <Select 
              ...
              {/* 音频配置 */}
              {config.modalityType?.includes('audio') && (
                <div className={`space-y-4 ${(config.modalityType?.includes('image') || config.modalityType?.includes('video')) ? 'pt-4 border-t border-white/10' : ''}`}>
                  <h4 className="text-sm font-semibold text-gray-300 flex items-center gap-2">
