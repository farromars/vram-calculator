...
import { Gpu, Zap, AlertTriangle, CheckCircle, Star, MousePointerClick, Cpu } from 'lucide-react';
...
import { ParallelismAdvisor } from '@/components/parallelism-advisor';
import { ParallelismConfig } from '@/utils/parallelism-formulas';
...
export function GPURecommendations({ requiredMemoryGB, title, modelInfo, perfConfig }: GPURecommendationsProps) {
  ...
  // 并行方案页签下，由 ParallelismAdvisor 回调同步过来的 GPU 和配置
  const [parallelGpu, setParallelGpu] = useState<GPU | null>(null);
  const [parallelConfig, setParallelConfig] = useState<ParallelismConfig | null>(null);
  ...
  const activeGpuForPerf = useMemo((): GPU | null => {
    ...
    return {
      ...
      bandwidth: (activeGpu.bandwidth || 0) * gpuCount * 0.85,
      fp16Tflops: (activeGpu.fp16Tflops || 0) * gpuCount * 0.80,
    };
  }, [activeGpu, gpuCount]);

  // 并行方案页签：ParallelismAdvisor 回调，更新"性能与内存结果"
  const handleParallelConfigChange = useCallback((gpu: GPU, config: ParallelismConfig) => {
    setParallelGpu(gpu);
    setParallelConfig(config);
  }, []);

  // 当前页签对应的 GPU（用于 PerformanceMemoryCard）
  const currentPerfGpu = activeTab === 'parallel' ? parallelGpu : activeGpuForPerf;
  const currentRequiredMemory = activeTab === 'parallel' && parallelConfig
    ? requiredMemoryGB  // 需求不变，只是 GPU 容量变了
    : requiredMemoryGB;
  ...
  return (
    ...
    >
      ...
      {/* 性能与内存结果卡片 — 始终显示，根据当前页签切换 GPU */}
      {currentPerfGpu && modelInfo && perfConfig && (
        <PerformanceMemoryCard
          gpu={currentPerfGpu}
          modelInfo={modelInfo}
          perfConfig={perfConfig}
          requiredMemoryGB={currentRequiredMemory}
        />
      )}

      {/* 页签切换 */}
      <div className="flex gap-0 mb-6 rounded-xl overflow-hidden border border-white/20">
          <button
            onClick={() => setActiveTab('gpu')}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 text-sm font-medium transition-all ${
              activeTab === 'gpu'
                ? 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-500'
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 text-sm font-medium transition-all ${
              activeTab === 'parallel'
                ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400 border-b-2 border-amber-500'
                : 'text-gray-500 hover:bg-white/10 hover:text-gray-300'
            }`}
          >
        ...
        {activeTab === 'parallel' && (
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  所需显存 {formatMemorySize(requiredMemoryGB)} — 选择下方 GPU 和节点配置后，「性能与内存结果」将同步更新
                </div>
              </div>
            </div>
            ...
            <ParallelismAdvisor
              ...
              isMoE={modelInfo?.architecture === 'moe'}
              seqLen={perfConfig?.sequenceLength ?? 4096}
              onConfigChange={handleParallelConfigChange}
            />
          </motion.div>
        )}
      </AnimatePresence>
