...
本项目是一个**GPU显存需求预估工具**，目标用户是在云平台上部署/训练/微调大模型的开发者。用户在购买GPU资源前，可通过本工具预估所需显存，避免资源浪费或不足。

### 1.2 接入整体步骤

```
步骤1: 静态部署
  将 npm run build 产物部署到 EdgeOne Pages、S3 或 CDN
...
步骤3: UI 风格对齐
  已完成现代设计体系适配（品牌蓝 #0052D9）
  导航栏、卡片、按钮均使用 tc-* 样式类

步骤4: 数据联动
  GPU 数据库已涵盖主流数据中心及消费级 GPU 规格
  数据来源: https://cloud.tencent.com/document/product/851/74108
  未来可通过 API 动态拉取云厂商实时机型和价格
...
│  └─ tailwind.config.js 中定义了品牌色
│
├─ TDesign React ───── 腾讯开源组件库
│  └─ 引入了全局CSS变量
│
├─ Radix UI ─────────── 无障碍基础组件
│  ├─ Select ── 下拉选择器
...
| `src/lib/models-data.ts` | 数据库 | 130+模型参数 + 15个主流GPU规格 |
...
**不是。** plan.md 中的 Agent Extensions 仅为计划阶段的建议工具，实际未调用。所有计算公式来自原始开源项目的计算引擎，GPU数据来自芯片厂商及云厂商官方文档。
...
适合度评分范围为 **0～100 分**，分两种情况计算：

**满足显存需求时**（总显存 ≥ 所需显存）：

```
fitScore = 100 - (空闲显存 / 总显存) × 50
         = 100 - ((totalMem - requiredGB) / totalMem) × 50
```

得分区间：**50～100 分**，利用率越高、浪费越少，得分越接近 100。

| 显存利用率 | 示例 | 得分 |
|-----------|------|------|
| 100%（刚好装满）| 需求 80GB，总显存 80GB | **100** |
| 80% | 需求 80GB，总显存 100GB | **90** |
| 50% | 需求 80GB，总显存 160GB | **75** |
| ~0%（极大浪费）| 趋近 | **50** |

**不满足显存需求时**（总显存 < 所需显存）：

```
fitScore = max(0, (totalMem / requiredGB) × 50)
```

得分区间：**0～50 分**，永远低于任何满足需求的卡。差距越大得分越低。
...
1. **满足需求的卡排前面**，不满足的排后面
2. 同样满足需求的卡中，按**利用率距离 80% 最近**排序（最优区间为 70%～90%）
3. 同样不满足需求的卡，按**显存容量从大到小**排序

### 7.3 设计理念与局限

**核心理念**："刚好够用最好"——利用率越高代表资源匹配越精准，不浪费显存。

**当前局限**：评分仅考虑显存容量匹配度，**未纳入算力（fp16Tflops）和带宽（GB/s）**。因此在同等满足显存需求的情况下，一张 80GB 老卡（利用率 95%，得分 98）可能排在一张 141GB 新卡（利用率 57%，得分 79）前面，但实际推理速度差异显著。后续可考虑引入算力加权因子优化评分模型。
